...
class Inference_Analyze(object):
    ...
    # @param self.num_dag: map from node to the number of apperearance of the node
    ...
    #
    # @return 
    def mark_search_space(self,viz_graph,sub_graph,dag, dag_id,
        ...
        for arg in dag.args:
            if  True:
                ...
                ## not in the right path 
                    ...
                    self.mark_search_space(viz_graph,sub_graph, arg, new_arg_id, False, simplify, proof_node)
    ...
    def print_other(self, results, step_count):
        ...
        log.info("\n" )
        log.info("****************** number of nodes in search space tree: %s*************************(not display target node)"%(self.node_no))
        t = 0
